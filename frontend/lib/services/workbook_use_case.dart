...
/// Workbook 데이터 변환 UseCase
/// 
/// API 응답을 도메인 엔티티로 변환하고, UI 모델로 변환합니다.
/// className은 이미 Repository에서 조회되어 전달됩니다.
  ...
  Future<Map<int, List<WorkbookSummaryEntity>>> convertApiResponsesToSummaries(
    ...
    for (final apiResponse in apiResponses) {
      ...
      for (final bookData in apiResponse.books) {
        ...
        if (bookData.latestUpdatedAt != null && bookData.latestUpdatedAt!.isNotEmpty) {
          try {
