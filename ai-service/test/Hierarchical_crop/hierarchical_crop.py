...
class ResNetClassifier:
    ...
    def __init__(self, model_path: str, model_type: str = 'answer', device: str = 'cuda'):
        ...
        logger.info(f"✅ ResNet 모델 로드 완료 ({model_type}): {model_path}")
    ...
    def predict(self, image_path: str) -> Tuple[Optional[str], float]:
        ...
        try:
            ...
            # logger.info(f"ResNet ({self.model_type}) 예측: 답안={answer}, 신뢰도={conf:.4f}")  # 상세 로그 비활성화
            
            return answer, conf
            
        except Exception as e:
            logger.error(f"ResNet ({self.model_type}) 예측 실패: {e}", exc_info=True)
            return None, 0.0
...
class HierarchicalCropPipeline:
    ...
    def get_missing_problem_number(self, page_number: Optional[str], 
        ...
        Args:
            page_number: 페이지 번호
            recognized_numbers: 이미 인식된 문제 번호 set
            
        Returns:
            Optional[str]: 누락된 가장 작은 문제 번호, 또는 None (답지가 없거나 누락 없음)
        """
        ...
        # 누락된 번호 중 가장 작은 것 (문자열을 정수로 변환하여 정렬)
        try:
            ...
            logger.info(f"   전체 누락: {sorted(missing_sorted)}")
    ...
    def visualize_section_detection(self, image_path: str, section_bbox: List[float], 
        ...
        ✨ section, answer_1, answer_2, 1-5 모든 검출 결과를 시각화
        ...
        Args:
            ...
            output_dir: 출력 디렉토리
            page_name: 페이지 이름
            detections_1104: 1104 모델의 검출 결과 (answer_1, answer_2, 1-5)
        ...
        colors = {
            'answer_1': (255, 0, 0),      # 빨강
            'answer_2': (255, 0, 255),    # ✨ 자홍 (answer_2 추가)
            '1': (0, 0, 255),              # 파랑
            '2': (255, 165, 0),            # 주황
            '3': (255, 255, 0),            # 노랑
            '4': (128, 0, 128),            # 보라
        ...
        # Section 내의 answer_1, answer_2, 숫자 검출 결과 그리기
        ...
        legend_y = 30
        legend_x = w - 200
        cv2.rectangle(image, (legend_x - 10, 10), (w - 10, 220), (255, 255, 255), -1)  # 높이 증가
        cv2.rectangle(image, (legend_x - 10, 10), (w - 10, 220), (0, 0, 0), 2)
        ...
        logger.info(f"✨ Section 검출 시각화 저장 (answer_1 + answer_2 + 1-5 포함): {save_path}")
    ...
    def crop_page_number(self, image_path: str, detections_1004: List[Dict], output_dir: Path) -> Tuple[Optional[str], Optional[str]]:
        ...
        Returns:
            Tuple[Optional[str], Optional[str]]: (None, 인식된 페이지 번호)
        """
        image = cv2.imread(image_path)
        if image is None:
            logger.error(f"이미지 로드 실패: {image_path}")
        ...
        # logger.info(f"페이지 번호 OCR 결과: '{recognized_number}' (이미지 저장 안 함)")  # 상세 로그 비활성화
    ...
    def classify_answer_with_resnet(self, answer_crop_path: str, section_idx: int, 
        ...
        Args:
            answer_crop_path: answer crop 이미지 경로
            section_idx: Section 인덱스
            answer_type: 답안 타입 ('answer_1' 또는 'answer_2')
            
        Returns:
            Tuple[Optional[str], float, bool]: (예측 답안, 신뢰도, 실패 여부)
        """
        # answer_type에 따라 적절한 모델 선택
        ...
        # logger.info(f"Section {section_idx}: ✨ {answer_type} ResNet 분류 시도 중...")  # 상세 로그 비활성화
        ...
        try:
            ...
            # logger.info(f"Section {section_idx}: ✅ {answer_type} ResNet 예측 성공 - 답안={answer}, 신뢰도={confidence:.4f}")  # 상세 로그 비활성화
    ...
    def process_single_section(self, original_image_path: str, section_idx: int, 
        ...
        """섹션 내의 문제번호와 정답을 추론
        ✨ 새로운 로직: 
        1. YOLO로 answer_2 검출 여부 확인
        2. answer_2 있으면 → answer_2 ResNet만 사용 (최종 답)
        3. answer_2 없으면 → answer_1 ResNet + YOLO IoU 검증 (기존 로직)
        4. 문제번호가 2개 검출된 경우 → 답지 기반 누락된 가장 작은 번호 할당
        ✨ Section 이미지만 저장, 다른 이미지들은 저장하지 않음
        ...
        Returns:
            Dict: {'section_idx', 'problem_number_ocr', 'answer_number', 'section_crop_path', 'problem_crop_path', 
                   'answer_1_crop_path', 'answer_2_crop_path', 'crop_success', 'crop_failure_reason', 
                   'used_resnet', 'resnet_confidence', 'resnet_failed', 'answer_source'}
        """
        ...
        try:
            ...
            # logger.info(f"✅ Section {section_idx} 이미지 저장 성공 (padding {self.section_padding}px 추가): {section_crop_path}")  # 상세 로그 비활성화
        ...
        # 1104 모델에서 answer_1, answer_2, 숫자(1-5) 검출 (확장된 영역 기준)
        section_dets_1104 = []
        for det in detections_1104:
            dx1, dy1, dx2, dy2 = det['bbox']
            cx, cy = (dx1 + dx2) / 2, (dy1 + dy2) / 2
        ...
        # 디버깅: answer_1과 answer_2 검출 결과 (상세 로그 비활성화)
        ...
        # 
        # logger.info(f"Section {section_idx}: 1104 모델 answer_2 검출 개수 = {len(answer_2_dets)}")
        # if answer_2_dets:
        #     for i, det in enumerate(answer_2_dets):
        #         logger.info(f"  answer_2 {i+1}: confidence={det['confidence']:.3f}, bbox={det['bbox']}")

        # ✨ 문제번호 OCR (이미지 저장 없이 OCR만 수행)
        problem_ocr = None
        problem_crop_path = None
        problem_ocr_override = False  # ✨ 답지 기반 할당 여부
        
        if section_dets_1004:
            # ✨ 문제번호가 2개 검출된 경우 처리
            if len(section_dets_1004) == 2:
                logger.warning(f"⚠️ Section {section_idx}: 문제번호 2개 검출됨!")
                logger.warning(f"   답지 기반으로 누락된 가장 작은 번호를 할당합니다.")
                
                # 답지에서 누락된 가장 작은 문제 번호 찾기
                ...
                if missing_number:
                    ...
                    logger.warning(f"Section {section_idx}: 누락된 문제 번호를 찾을 수 없음, 신뢰도 높은 것 사용")
                    best_det = max(section_dets_1004, key=lambda x: x['confidence'])
            else:
                best_det = max(section_dets_1004, key=lambda x: x['confidence'])
            
            # 문제번호 OCR 수행 (이미지 저장 안 함)
            if not problem_ocr_override:
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    # 임시 파일로 OCR 수행 (저장하지 않음)
                    import tempfile
                    with tempfile.NamedTemporaryFile(suffix='.jpg', delete=True) as tmp:
                        cv2.imwrite(tmp.name, cropped)
                        problem_ocr = self.ocr.extract_number(tmp.name)
                    
                    # logger.info(f"Section {section_idx} 문제번호 OCR 결과: '{problem_ocr}' (이미지 저장 안 함)")  # 상세 로그 비활성화
        else:
            logger.warning(f"Section {section_idx}: 문제번호 미검출")

        # ✨✨✨ 새로운 로직: YOLO 검출 결과 기반 처리 ✨✨✨
        best_answer = None
        used_resnet = False
        resnet_confidence = 0.0
        resnet_failed = False
        answer_1_crop_path = None
        answer_2_crop_path = None
        answer_source = None  # 'answer_2_resnet', 'answer_1_resnet', 'answer_1_yolo'
        ...
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        if answer_2_dets:
            # logger.info(f"Section {section_idx}: 🎯 answer_2 검출됨 - answer_2 ResNet만 사용")  # 상세 로그 비활성화
            
            # answer_2를 crop (임시 파일, 저장하지 않음)
            ...
            if x2 > x1 and y2 > y1:
                ...
                try:
                    ...
                    if not failed and answer is not None:
                        ...
                        # logger.info(f"Section {section_idx}: ✅ answer_2 ResNet 성공 - 답안={best_answer}, 신뢰도={conf:.4f} (최종 답)")  # 상세 로그 비활성화
                    else:
                        resnet_failed = True
                        used_resnet = True
                        answer_source = 'answer_2_failed'
                        logger.error(f"Section {section_idx}: ❌ answer_2 ResNet 실패")
                finally:
                    # 임시 파일 삭제
                    os.unlink(answer_2_temp_path)
            else:
                logger.warning(f"Section {section_idx}: answer_2 bbox가 유효하지 않음")
                answer_source = 'answer_2_invalid_bbox'
        
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        # 2단계: answer_2가 없거나 실패 → answer_1 처리 (기존 로직)
        # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
        if best_answer is None:
            # logger.info(f"Section {section_idx}: 🔄 answer_2 없음 또는 실패 → answer_1 처리")  # 상세 로그 비활성화
            
            if answer_1_dets:
                # answer_1을 crop (임시 파일, 저장하지 않음)
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    try:
                        ...
                        if not failed and answer is not None:
                            ...
                            if number_dets:
                                ...
                                if answer != yolo_best_number:
                                    ...
                                    if conf < 0.70 and yolo_best_iou > 0.3:
                                        # logger.warning(f"   → ResNet 신뢰도 낮음({conf:.2f}), YOLO 결과({yolo_best_number}) 사용")  # 상세 로그 비활성화
                                        best_answer = yolo_best_number
                                        answer_source = 'answer_1_yolo'
                                    else:
                                        # logger.info(f"   → ResNet 신뢰도 높음({conf:.2f}), ResNet 결과({answer}) 유지")  # 상세 로그 비활성화
                                        best_answer = answer
                                        answer_source = 'answer_1_resnet'
                                else:
                                    # logger.info(f"✅ Section {section_idx}: ResNet과 YOLO IoU 일치 ({answer})")  # 상세 로그 비활성화
                                    best_answer = answer
                                    answer_source = 'answer_1_resnet'
                            else:
                            ...
                            used_resnet = True
                            resnet_failed = False
                            # logger.info(f"Section {section_idx}: ✅ 최종 답안={best_answer} (출처: {answer_source})")  # 상세 로그 비활성화
                        else:
                            resnet_failed = True
                            used_resnet = True
                ...
                logger.warning(f"Section {section_idx}: answer_1과 answer_2 모두 검출되지 않았습니다.")
                resnet_failed = True
                answer_source = 'no_answer_detected'

        # Section 검출 시각화 저장 (✨ 모든 검출 정보 포함) - 비활성화
        ...
        return {
            ...
            'problem_number_override': problem_ocr_override,  # ✨ 답지 기반 할당 여부 플래그
            'answer_number': best_answer,
            'section_crop_path': str(section_crop_path),
            'problem_crop_path': None,  # ✨ 더 이상 저장하지 않음
            'answer_1_crop_path': None,  # ✨ 더 이상 저장하지 않음
            'answer_2_crop_path': None,  # ✨ 더 이상 저장하지 않음
            'crop_success': True,
            ...
            'answer_source': answer_source,  # ✨ 답안 출처 추가
            ...
            'debug_info': {
                ...
                'answer_2_count': len(answer_2_dets),  # ✨ answer_2 개수 추가
                'problem_number_count': len(section_dets_1004)  # ✨ 문제번호 검출 개수
            }
        }
    ...
    def process_page(self, image_path: str, output_dir: Path) -> Optional[Dict]:
        """페이지 처리: 1104 모델로 answer_1/answer_2 검출, answer_2 우선 처리
        ...
        # 페이지 번호 OCR (1004 모델 사용, 이미지 저장 안 함)
        ...
        # 1104 모델: answer_1, answer_2 검출용
        logger.info("1104 모델 실행 중 (answer_1, answer_2 검출)...")
        ...
        for idx, det in enumerate(section_detections):
            ...
            if section_result.get('problem_number_ocr'):
                ...
                # logger.info(f"📝 인식된 문제 번호 추가: {section_result['problem_number_ocr']}")  # 상세 로그 비활성화
                # logger.info(f"   현재까지 인식된 문제: {sorted(recognized_problem_numbers)}")  # 상세 로그 비활성화
