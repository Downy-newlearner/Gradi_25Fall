...
def load_answers(answer_file="answer.txt"):
    """
    ...
    with open(answer_file, encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            page, num, ans = [x.strip() for x in line.split(",")]
            # (page, num) 기반으로 실제 정답 저장
...
def load_problems():
    ...
    JSON 파일 구조:
        ...
        - korean_content: 한글 문제 내용
        - english_content: 영어 지문
        - script: 듣기 대본 
        - answer_option: 선택지
    """
    current_dir = os.getcwd()
    folder_path = os.path.join(current_dir, "database_file")
...
def generate_explanation(user_input):
    ...
    Args:
        user_input (str): 사용자 입력 문자열
            형식: "{페이지}페이지 {문제번호}번 문제는 {답안}번이다."
            예시: "102페이지 40번 문제는 5번이다."
            
            파싱 규칙:
            - 페이지: 숫자 (예: 102)
            - 문제번호: 숫자, 2자리로 zero-padding (예: 40 → "40", 1 → "01")
            - 학생 답안: 숫자 (예: 5)
    ...
    Raises:
        None (에러 시 에러 메시지 문자열 반환)
        
    예시:
        >>> result = generate_explanation("102페이지 40번 문제는 5번이다.")
        >>> print(result)
        ------------------------------------
        🔎 문제 40 (p.102)
        ✅ 정답: 1  
    ...
    real_answers = load_answers()
    
    # 사용자 입력 파싱: "{페이지}페이지 {문제번호}번 문제는 {답안}번이다."
    input_match = re.search(r"(\d+)페이지\s+(\d+)번.*?(\d+)번", user_input)
    if not input_match:
        return "사용자 입력을 인식할 수 없습니다. 형식: '{페이지}페이지 {문제번호}번 문제는 {답안}번이다.'"
    
    page, num, user_ans = input_match.groups()
    num = num.zfill(2)  # 문제번호를 2자리로 zero-padding (예: 1 → "01")
    
    # 해당 문제 검색
    target = None
    for prob in problems:
        if str(prob.get("page_number")) == page and int(prob.get("problem_number")) == int(num):
            target = prob
            break
...
if __name__ == "__main__":
    # 테스트 예시
    test_input = "102페이지 40번 문제는 5번이다."
    result = generate_explanation(test_input)
    print(result)